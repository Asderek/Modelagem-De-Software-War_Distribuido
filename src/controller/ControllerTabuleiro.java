...
public class ControllerTabuleiro extends Observable {
	...
	private static List<model.Exercito> lstJogadores;
	...
	private void proxJogador() throws IOException {
	...
	public boolean possuiTerritorio(Exercito e) {
		...
		if (e.isJogadorFora()) {
			ArrayList<Carta> lstCartaAux = new ArrayList<Carta>(e.getLstCartas());
	 ...
	 * 
	 */
	public void preparaTabuleiro() throws IOException {
	...
	/*asd*/
	private void proxJogada() throws IOException {
		notificaMudancas();
	...
	public static List<model.Exercito> getLstJogadores() {
		if (lstJogadores == null)
			lstJogadores = new ArrayList<model.Exercito>(); 
		return lstJogadores;
	}
	
	public void setLstJogadores(String[] jogadores)
	{
...
//		{
//			setJogador("Laranja", 	new Color(209, 84, 000));
//			
//			
//			
...
//				break;
//			case "vermelho":
//				setJogador("Vermelho",	new Color(255, 003, 003));
//				break;
//			case "azul":
//				setJogador("Azul", 		new Color(030, 067, 186));
//				break;
//			case "verde":
//				setJogador("Verde", 	new Color(006, 124, 000));
//				break;
//			case "amarelo":
//				setJogador("Amarelo", 	new Color(255, 188, 000));
//				break;
//			case "preto":
//				setJogador("Preto", 	new Color(000, 000, 000));
//				break;
//			default:
//				break;
//			}
//			
//		}
//		System.out.println("Depois - lstJogadores.size = "+lstJogadores.size());
//		System.out.println("ItJogador = "+itJogador);
//		itJogador = lstJogadores.iterator();
//		System.out.println("New ItJogador = "+itJogador);
		
		try {
			Configuracao.getInstance().validaJogadores();
		} catch (IOException e) {
	...
	public static void setJogador(String s, Object cor) {
		getLstJogadores().add(new Exercito(s, cor));
	}
	...
	/*asd*/
	public void btnProxJogada_click() throws IOException {
		
		boolean changedTurn=jogadorDaVez.getNome().equals(Client.getInstance().getColor());
		if(!itJogada.hasNext()&&changedTurn)
		{
			if (War.client!=null)
			{
		...
		System.out.println("Antes da Troca de Jogadores: "+ jogadorDaVez.getNome());
		if (vencedor == null) {
			setTerritorioDestino(null);
			setTerritorioOrigem(null);
			proxJogada();
		}
		System.out.println("Depois da Troca de Jogadores: "+ jogadorDaVez.getNome());
//		if(getJogadaAtual().getNome() == "Distribuir")
//		{
		if (changedTurn)
		{
				
				Client.getInstance().sendMessage("JogadorDaVez -"+ jogadorDaVez.getNome());
		}
	...
	public void FimDaJogada() throws IOException {
		
		System.out.println("Sending Fim da Jogada" + lstContinentes.size());
		Client.getInstance().sendMessage("FimDaJogada");
		
		
		for (Continente c: lstContinentes)
		{
			Client.getInstance().sendMessage(Integer.toString(c.getLstTerritorios().size())); //numTerritorios
//			System.out.println(Integer.toString(c.getLstTerritorios().size()));
			
			for (Territorio t: c.getLstTerritorios())
			{
//				System.out.println(t.getNome());
//				System.out.println("\t"+Integer.toString(t.getLstSoldados().size()));
//				System.out.println("\t"+t.getLstSoldados().get(0).getExercito().getNome());
				Client.getInstance().sendMessage(t.getNome());
				Client.getInstance().sendMessage(Integer.toString(t.getLstSoldados().size()));
				Client.getInstance().sendMessage(t.getLstSoldados().get(0).getExercito().getNome());
			}
			
//			System.out.println("----End of Continent----");
	...
	public void pnlMapa_click(int x, int y, int botaoMouse) {
		...
		try {
			if (Client.getInstance().getColor().equals(ControllerTabuleiro.getInstance().getJogadorDaVez().getNome())){
				if (vencedor == null) {
	...
	public void UpdateTabuleiro(ArrayList<Continente> novaLst)
	{
		Continente continenteNovo=null;
		Territorio territorioNovo=null;
		

		for (Continente continenteAntigo: lstContinentes)
		{
			
			for (int i = 0;i<novaLst.size();i++)
			{
				if(continenteAntigo.getNome().equals(novaLst.get(i).getNome()))
					continenteNovo = novaLst.get(i);
			}
			
			
			/*teste*/
			
			for (Territorio TAntigo: continenteAntigo.getLstTerritorios())
			{
				
				for (int i = 0;i<continenteNovo.getLstTerritorios().size();i++)
				{
					if(TAntigo.getNome().equals(continenteNovo.getLstTerritorios().get(i).getNome()))
					{
						territorioNovo = continenteNovo.getLstTerritorios().get(i);
						
					}
				}
				
				
				while (TAntigo.getLstSoldados().size()>1)
				{
					TAntigo.getLstSoldados().remove(0);
				}

				for (int i=0;i<territorioNovo.getLstSoldados().size();i++)
				{
					Exercito e = descobreExercito(territorioNovo.getDono());
					Soldado s = new Soldado(e);
					TAntigo.getLstSoldados().add(s);
